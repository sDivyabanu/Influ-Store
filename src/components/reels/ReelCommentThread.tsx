...
/** Mirrors components/posts/CommentThread.tsx for reel comments. */
export function ReelCommentThread({ comment, reelId, isReelOwner, onDeleted }: ReelCommentThreadProps) {
  ...
  async function loadReplies() {
    ...
    try {
      const res = await fetch(`/api/reel-comments/${comment.id}/replies?limit=${REPLIES_PAGE_SIZE}`);
  ...
  async function loadMoreReplies() {
    ...
    try {
      const res = await fetch(
        `/api/reel-comments/${comment.id}/replies?cursor=${encodeURIComponent(repliesCursor)}&limit=${REPLIES_PAGE_SIZE}`
      );
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error();
