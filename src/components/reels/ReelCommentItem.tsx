...
interface ReelCommentItemProps {
  ...
  /** True when the current user owns the reel this comment belongs to. */
  canModerate: boolean;
  onDeleted: (commentId: string) => void;
  onReplyClick?: () => void;
}

/** Mirrors components/posts/CommentItem.tsx, pointed at the reel-comment endpoints. */
export function ReelCommentItem({ comment, canModerate, onDeleted, onReplyClick }: ReelCommentItemProps) {
  ...
  async function toggleLike() {
    ...
    try {
      const res = await fetch(`/api/reel-comments/${comment.id}/like`, {
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      const res = await fetch(`/api/reel-comments/${comment.id}`, { method: "DELETE" });
      const data = await res.json();
